...
IMAGE_URL = 'https://peopledotcom.files.wordpress.com/2018/12/books-8.jpg?crop=0px%2C13px%2C2700px%2C1419px&resize=1200%2C630'  #@param {type:"string"}
...
background = cv2.imread("background2.jpg",1)
...
while(True):
    ...
    seg_image_color= label_to_color_image(seg_map).astype(np.uint8)
    image = cv2.resize(image,(seg_image_color.shape[1],seg_image_color.shape[0]))
    #print(image.shape)
    #print(seg_image_color.shape)
    added_image = cv2.addWeighted(image,0.4,seg_image_color,0.2,0)

    masked_image = cv2.bitwise_and(np.uint8(image), np.uint8(image), mask=np.uint8(seg_map))

    # Green screen https://docs.opencv.org/3.2.0/d0/d86/tutorial_py_image_arithmetics.html
    background = cv2.resize(background,(seg_image_color.shape[1],seg_image_color.shape[0]))
    ret, mask = cv2.threshold(np.uint8(seg_map), 10, 255, cv2.THRESH_BINARY)
    mask_inv = cv2.bitwise_not(mask)
    crop_background = cv2.bitwise_and(background,background,mask = mask_inv)


    res = crop_background + masked_image
    #added_image = cv2.addWeighted(masked_image,1,background,0.2,0)
